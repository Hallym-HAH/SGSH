...
export default function ReserveListApprove() {
    ...
    useEffect(() => {
        const fetchDate = async () => {
            ...
            if (user) {
                ...
                const { data: reserveData } = await supabaseClient.from('reserve_data').select(`*`).eq('b_id', b_id).eq('status', "approve").order("date", { ascending: true }).order("time", { ascending: true });
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                        {
                            reserved != null && reserved.length > 0 ? reserved.map((reserve, index) => (
                                <div key={index} className="flex w-full border border-green-300 rounded-lg my-2">
